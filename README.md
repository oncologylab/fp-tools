...
The manuscript normalization panel is regenerated from real demo B/T corrected cut-site matrices around IRF4- and ETS1-associated sites; the script creates two deterministic, modestly depth-offset replicate profiles per condition to stress-test sample-level quantile normalization without introducing unrealistic day-and-night sample differences. IRF4 is used as a biologically plausible B-cell-deeper example and ETS1 as a T-cell-deeper example from the same fixture set.
The same script also writes `paper/manuscript/figures/normalization_effect_candidates.png`, a direction-labelled contact sheet with additional B-cell-deeper and T-cell-deeper candidates after normalization.
