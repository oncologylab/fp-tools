...
class InteractiveDiffFootprintsHtmlTest(unittest.TestCase):
    def test_aggregate_payload_is_serialized_as_compressed_json(self):
        ...
        self.assertIn("function legendGroups", html)
        self.assertIn('class="legend-group"', html)
        self.assertIn("aggregateLegendLineWidth=3", html)
        self.assertIn('stroke-width="${aggregateLegendLineWidth}"', html)
        self.assertIn("aggregate-export-legend", html)
        self.assertIn("legendH+Math.floor", html)
