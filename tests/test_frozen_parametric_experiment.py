...
def test_residual_selector_uses_difficult_tasks_and_ctcf_gate() -> None:
    ...
    for residual, difficult_ap, ctcf_auc in (
        ...
        rows.extend(
            [
                {
                    "cell": "K562", "tf": "MEF2A", "role": "difficult",
                    "method": f"factorized_residual_{residual}", "auroc": 0.7,
                    "auprc": difficult_ap,
                },
                {
                    "cell": "K562", "tf": "CTCF", "role": "positive_control",
                    "method": f"factorized_residual_{residual}", "auroc": ctcf_auc,
                    "auprc": 0.8,
                },
            ]
        )
    rows.extend(
        [
            {"cell": "K562", "tf": "MEF2A", "role": "difficult", "method": "DWM", "auroc": 0.6, "auprc": 0.5},
            {"cell": "K562", "tf": "CTCF", "role": "positive_control", "method": "DWM", "auroc": 0.8, "auprc": 0.8},
        ]
    )
    selected, summary = evaluate_parametric_factorization.select_residual(pd.DataFrame(rows))
    # Pearson has the best difficult-task AP but violates the -0.02 CTCF gate.
    assert selected == "deviance"
    assert summary.loc[summary["residual"] == "pearson", "passes_ctcf_gate"].item() is False
...
def test_stage_runner_completes_and_resumes_synthetic(tmp_path: Path) -> None:
    runner = run_frozen_parametric_experiment.StageRunner(
        study_path=ROOT / "benchmarks/manifests/frozen_parametric_factorization_v1.spec.json",
        holdout_freeze_path=ROOT / "benchmarks/manifests/frozen_parametric_factorization_v1.freeze.json",
        registry_path=ROOT / "benchmarks/manifests/frozen_parametric_local_inputs_v1.json",
        outdir=tmp_path / "run",
    )
    assert runner.run_stage("synthetic") == "completed"
    assert runner.run_stage("synthetic") == "resumed_verified"
    completion = json.loads((tmp_path / "run/synthetic/stage.complete.json").read_text())
    assert completion["schema"] == run_frozen_parametric_experiment.STAGE_COMPLETE_SCHEMA
