...
class ReviewMultiComparisonsTest(unittest.TestCase):
    ...
    def test_command_writes_aggregate_free_self_contained_html(self):
        ...
        self.assertEqual(document.count('id="comparison-selector"'), 1)
        self.assertIn('id="comparison-selector-control" hidden', document)
        self.assertIn('class="aggregate-card aggregate-only"', document)
        self.assertIn("body.no-aggregate .aggregate-only", document)
        self.assertIn("function drawRank", document)
        self.assertIn("function renderVolcano", document)
        self.assertIn("function logoPanelSvg", document)
        self.assertIn("state.hasAggregates", document)
        self.assertNotIn('id="panel-count"', document)
    ...
    def test_aggregate_controls_remain_visible_when_profiles_exist(self):
        ...
        self.assertIn('class="aggregate-card aggregate-only"', document)
        self.assertIn("function renderAggregateGrid", document)
    ...
    def test_hidden_legacy_panel_options_do_not_change_standalone_selector(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            inputs = []
            for idx in range(8):
        ...
        self.assertEqual(html.count('id="comparison-selector"'), 1)
        self.assertNotIn('id="panel-count"', html)
