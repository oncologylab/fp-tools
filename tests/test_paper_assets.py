...
class PaperAssetsTest(unittest.TestCase):
    def test_paper_templates_are_present(self):
        for rel_path in [
            "manuscript/templates/MDPI_template_ACS.zip",
            "manuscript/templates/biomedinformatics-template.dot",
            "manuscript/templates/BioMedInformatics",
        ]:
            self.assertTrue((ROOT / rel_path).exists(), rel_path)

    def test_mdpi_latex_template_zip_has_required_files(self):
        names = prepare_biomed.validate_template_zip(ROOT / "manuscript" / "templates" / "MDPI_template_ACS.zip")
    ...
    def test_prepare_template_creates_manuscript_workspace(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            prepare_biomed.prepare_template(ROOT / "manuscript" / "templates" / "MDPI_template_ACS.zip", outdir)
