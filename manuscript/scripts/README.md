...
- `manuscript/figures/*.pdf` and `*.svg` for manuscript submission.
- `manuscript/figures/*.png` for GitHub previews.
- `manuscript/tables/*.tsv` and `*.csv` for source data.

## Available Builders

- `plot_benchmark_panels.py`: render AUROC/AUPRC/recall/calibration benchmark summaries from metrics TSV files.
- `plot_multiscale_npz.py`: render PDF/SVG/PNG multi-panel summaries from `call-footprints --output-multiscale-npz` sidecars.
