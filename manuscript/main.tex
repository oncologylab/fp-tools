...
reproducibility practices. We present \texttt{fp-tools}, a standalone Python
...
and auditable platform for classical ATAC-seq footprinting workflows and their
...
single command-line workflow. The package also supports de novo motif-discovery
preparation, pseudobulk ATAC aggregation, and replicate-aware reporting, allowing
these analyses to be incorporated without changing the core signal-processing
path. This paper describes the implemented methods, public-data validation
...
\subsection{Optional Analysis Modules}

Several analysis modules extend the core footprinting workflow:
\begin{itemize}
  ...
  \item \emph{Replicate-aware differential footprint analysis} treats repeated samples from
...
Conceptually, the optional modules answer focused questions around the
...
converted to CPM-normalized cut-site bigWigs. For aggregate validation, exact
motif-centered site sets are generated by scanning JASPAR2026 CORE vertebrate
motifs inside the 10x ATAC peak set. A predeclared immune-regulator panel is
screened by expected-lineage center protection, and the displayed panels are
selected from that source table rather than from peak centers.
...
\texttt{pvalue} columns together with a
Benjamini--Hochberg-adjusted \texttt{qvalue\_bh} column and a
\texttt{significant\_fdr05} flag. The existing \texttt{highlighted} column is
retained as a report-selection and visualization flag, not as a formal
multiple-testing-corrected significance label. Additional replicate-oriented columns include replicate counts, condition score
SDs, mean $\Delta$, mean $L$, and standard-error summaries.
...
The benchmark is organized as a tiered design
(Table~\ref{tab:tiers}): a small-reference regression tier; a bulk matched
tier (ENCODE bulk ATAC vs. matched TF ChIP/CUT\&RUN) for classical
...
test, build, and manuscript-smoke workflows; detailed reviewer instructions are
...
In addition to direct command-line execution, \texttt{fp-tools} provides two
usability layers for reviewing results. The optional GUI installed with
\texttt{fp-tools-bio[gui]} builds the same normalized YAML configurations used by
\texttt{run-workflow}, so browser-based setup remains compatible with the command
line. Separately, the differential-footprint and aggregate-batch commands write
standalone HTML reports with embedded compressed data, searchable motif controls,
...
\caption{Usability-oriented \texttt{fp-tools} interfaces. (A) Optional browser GUI
for configuring command-line workflows and saving runnable YAML configurations.
(B) Standalone \texttt{diff-footprints} HTML report from the Buenrostro B-cell
...
\texttt{plot-aggregate-batch} HTML report generated from the same embedded
...
The classical workflow is covered by deterministic regression tests that pin
...
core analyses while allowing optional modules to share the same input and output
conventions.
...
\caption{De novo motif discovery used as standalone analysis and as a supplement
...
condition means are reported alongside individual replicate profiles, and optional
...
classical footprinting workflow produces stable outputs on small fixtures and
...
handling of replicate-aware summaries; and an extension model in which de novo motif preparation and pseudobulk aggregation are integrated without disrupting the classical workflow
...
classical footprinting workflow that can be run from corrected ATAC-seq signal
...
that unifies classical ATAC-seq footprinting with motif-discovery preparation, pseudobulk aggregation, and replicate-aware reporting.
By pairing stable, test-covered workflows with public-data manifests, source
tables, and reproducible figure generation, the package lowers the barrier to standardized,
