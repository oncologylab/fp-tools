...
The same PBMC5k footprint-signature table was used to prioritize cell-type
associated motif signatures for visualization. We selected top signatures with
clear broad-cell enrichment and included the representative TF rows used in the
UMAP panel. KNN-smoothed footprint-signature scores were then projected onto the
PBMC5k UMAP. For each cell, the displayed value summarizes local motif-centered
cut-site profiles in neighboring cells. The final figure combines a
marker-by-cell heatmap with UMAP panels for broad cell-type labels and
