...
compatibility. For each comparison, \texttt{fp-tools} also reports a
...
and standard-error summaries. A legacy replicate-report command remains
available as a post-hoc diagnostic helper for existing \texttt{*\_results.txt}
files.
...
benchmark scaffold; a depth-robustness tier over downsampled ATAC; a de novo motif-preparation tier;
...
small fixtures, builds source and wheel artifacts, and smoke-tests the current
console scripts from an installed wheel. The repository includes
...
\subsection{De Novo Motif Supplement Validation}

To test whether this utility adds practical value beyond a fixed motif database,
we ran a validation experiment on the same 2-vs-2 Buenrostro B-cell versus
...
novo-derived highlighted motifs. This result is not a claim that the recovered
motifs are novel TFs. Rather, it demonstrates the intended use case: when a
chosen motif database is incomplete or conservative, candidate-derived motifs can
be tested alone and can also be carried forward into the same differential
footprint report as database-supported motifs (Figure~\ref{fig:denovo_motif_validation}).

\begin{figure}[H]
\includegraphics[width=0.95\textwidth]{figures/denovo_motif_validation.png}
\caption{Validation of de novo motif discovery as standalone and supplement
modes. Candidate footprints from the Buenrostro B-cell versus T-cell replicate
...
motifs. Tomtom labels are motif-similarity annotations rather than definitive TF
identity calls. Thin aggregate lines show individual biological replicates and
...
motif hits were counted for each TF and only the top 500 scoring sites per TF
were used for compact plotting. The source screen required at least 500 natural
motif hits and 300 plotted sites, then ranked TFs by center protection in the
expected pseudobulk groups relative to other groups. This selected SPIB as the
B-cell-associated example, RUNX3 as the T/NK-associated example, and CEBPB and
CEBPA as myeloid-associated examples. The fragment data and pseudobulk cut-site
tracks were not modified; only the motif-centered site set used for aggregation
...
pseudobulk utility can also emit pseudo-paired BAMs for downstream bias correction
with zero read shifting. This result is intended as a support and reproducibility
demonstration, not as a full single-cell benchmark.
...
groups for each lineage-associated TF. The source screen, natural motif-hit counts,
and plotted-site counts are reported in
...
outputs. In the first-version scope, these resources
...
ChromBPNet on identical locus sets remain future full-benchmark work before any
broad, field-relative superiority claims. Larger public-benchmark inputs depend on external data acquisition, intentionally
