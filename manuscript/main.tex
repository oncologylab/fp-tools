...
and myeloid marker list. The screen required at least 200 motif-centered sites and
ranked TFs by center protection in the expected pseudobulk groups relative to other
groups. This selected BACH2 as the B-cell-associated example, RORA as the strongest
T/NK-associated example in this dataset, CEBPB as the myeloid-associated example,
and CTCF as a ubiquitous architectural control. The fragment data and pseudobulk
cut-site tracks were not modified; only the motif-centered site set used for
aggregation changed. The resulting profiles show that single-cell fragments can be
collapsed into group-level tracks and inspected with the same motif-centered
aggregate workflow used for bulk ATAC data (Figure~\ref{fig:pseudobulk_supp}).
...
\caption{Motif-centered 10x PBMC pseudobulk TF aggregate profiles.
...
JASPAR2026 motif centers scanned inside 10x ATAC peaks. BACH2, RORA, and CEBPB
were selected by a predeclared lineage screen as B-cell-associated, T/NK-associated,
and myeloid-associated examples, respectively; CTCF is shown as a high-site-count
ubiquitous control. Thick lines mark the expected pseudobulk groups for each
lineage-associated TF, while other groups are shown faintly. The source screen is
reported in \texttt{figures/supp\_pseudobulk\_tf\_screen.tsv}. The figure demonstrates
that the pseudobulk fragment workflow produces group-level signal tracks compatible
with motif-centered aggregate visualization.
