...
def center_condition_baselines(profiles: dict[str, np.ndarray], xvals: np.ndarray) -> dict[str, np.ndarray]:
    """Center normalized display profiles on each condition's distal baseline."""

    centered = dict(profiles)
    mask = np.abs(xvals) >= 45
    for condition in CONDITION_COLORS:
        baseline = baseline_mean(condition_mean(profiles, condition), xvals)
        for sample in condition_members(condition):
            centered[sample] = profiles[sample] - baseline
    return centered
...
def main(argv: list[str] | None = None) -> int:
    ...
    for tfbs in tfbs_list:
        ...
        norm_matrices = normalize_matrices(raw_matrices)
        raw_profiles = profiles_from_matrices(raw_matrices)
        norm_profiles = center_condition_baselines(profiles_from_matrices(norm_matrices), xvals)
        raw_summary = summarize(raw_profiles, xvals)
        norm_summary = summarize(norm_profiles, xvals)
    ...
    for row, (tfbs, _raw_matrices, _norm_matrices, raw_profiles, norm_profiles, raw_summary, norm_summary, ylimits) in enumerate(panels):
        ...
        axes[row, 0].text(0.02, 0.96, "two depth-offset replicates per condition", transform=axes[row, 0].transAxes, va="top", fontsize=7.2)
        ...
        axes[row, 1].text(0.02, 0.96, "normalized, baseline-centered display", transform=axes[row, 1].transAxes, va="top", fontsize=7.2)
