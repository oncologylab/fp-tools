...
paper-ready figure generators. In an end-to-end demonstration on ENCODE ATAC-seq,
the harness resolves a significant, reproducible gain from motif-aware scoring over
a raw accessibility baseline across six transcription factors in K562 (mean AUROC
$0.50\rightarrow0.78$) and two in GM12878, with non-overlapping bootstrap intervals;
it further shows that footprint occupancy is recoverable only at Tn5 cut-site
...
end-to-end on real data---and to quantify the benefit of motif-aware scoring over
raw accessibility---we benchmarked two predictors on ENCODE ATAC-seq peaks
restricted to chromosome~4, labeling each accessible region positive when it
overlaps a matched ChIP-seq peak for the target factor. The first predictor ranks
regions by their raw accessibility magnitude (peak signal value); the second is the
motif-aware score produced by scanning each region against the factor's JASPAR
motif over the hg38 sequence and taking the best two-strand log-odds match. We
evaluated six transcription factors in K562 (CTCF, GATA1, SPI1, REST, MAX, JUND;
$n=1{,}900$ accessible regions each) and, for cross-cell generalization, two
factors in GM12878 (CTCF, SPI1; $n=9{,}482$ regions).

Motif-aware scoring significantly outperforms the accessibility baseline for
\emph{every} factor tested, with non-overlapping 200-resample bootstrap confidence
intervals (Table~\ref{tab:k562}, Figure~\ref{fig:k562}). Across the six K562
factors the mean AUROC rises from $0.50$ (accessibility, i.e.\ chance) to $0.78$
(motif), with the largest gain for the long, information-rich CTCF motif
($0.53\rightarrow0.87$) and smaller but still significant gains for shorter or more
degenerate motifs such as GATA1 ($0.58\rightarrow0.72$) and REST
($0.48\rightarrow0.72$). The effect generalizes to a second cell type: in GM12878,
motif-aware scoring lifts CTCF AUROC from $0.57$ to $0.90$ and SPI1 from $0.50$ to
$0.78$, closely matching the corresponding K562 values. This confirms the expected
biology---total accessibility alone does not distinguish bound from unbound open
chromatin, while sequence/motif evidence does---and validates that the
\texttt{fp-tools} benchmark harness cleanly and reproducibly resolves real effects
between competing predictors across factors and cell types. Full genome-wide,
multi-cell benchmarks and external comparator runs are reported in the accompanying
benchmark release (see Data Availability) and are not reproduced here to keep large
generated outputs out of version control.

\begin{table}[H]
\caption{Demonstration bulk-matched benchmark on ENCODE ATAC-seq (chromosome~4),
ranking accessible regions by raw accessibility versus a motif-aware PWM score and
labeling by matched ChIP-seq overlap. AUROC values are point estimates with 200-
resample bootstrap 95\% CIs; motif-aware scoring significantly beats the baseline
for every factor and cell type.\label{tab:k562}}
\newcolumntype{Z}{>{\centering\arraybackslash}X}
\begin{tabularx}{\textwidth}{llZZ}
\toprule
\textbf{Cell / TF (motif)} & \textbf{Pos.\ frac.} & \textbf{Accessibility AUROC} & \textbf{Motif-aware AUROC}\\
\midrule
K562 / CTCF (MA0139.1) & 0.36 & 0.53 (0.51--0.56) & \textbf{0.87 (0.85--0.88)}\\
K562 / GATA1 (MA0035.4) & 0.08 & 0.58 (0.53--0.63) & \textbf{0.72 (0.68--0.76)}\\
K562 / SPI1 (MA0080.7) & 0.28 & 0.47 (0.45--0.51) & \textbf{0.79 (0.77--0.81)}\\
K562 / REST (MA0138.3) & 0.06 & 0.48 (0.42--0.54) & \textbf{0.72 (0.68--0.77)}\\
K562 / MAX (MA0058.4) & 0.49 & 0.44 (0.42--0.47) & \textbf{0.74 (0.72--0.76)}\\
K562 / JUND (MA0491.3) & 0.08 & 0.59 (0.54--0.64) & \textbf{0.83 (0.78--0.87)}\\
\midrule
GM12878 / CTCF & 0.26 & 0.57 (0.56--0.59) & \textbf{0.90 (0.89--0.91)}\\
GM12878 / SPI1 & 0.28 & 0.50 (0.49--0.52) & \textbf{0.78 (0.77--0.79)}\\
\bottomrule
\end{tabularx}
\end{table}

\begin{figure}[H]
\includegraphics[width=\textwidth]{figures/k562_method_comparison_multitf.png}
\caption{AUROC and AUPRC for a raw accessibility baseline (grey) versus motif-aware
scoring (blue) across six K562 transcription factors on ENCODE ATAC-seq peaks
(chromosome~4) labeled by matched ChIP-seq overlap. Motif-aware scoring exceeds the
accessibility baseline---and the chance line (red)---for every factor. Generated by
\texttt{paper/scripts/plot\_method\_comparison.py}.\label{fig:k562}}
\end{figure}
...
Limitations include the dependence of the public-benchmark results on external
data acquisition and comparator execution, which are intentionally kept outside
version control for size and licensing reasons; the tabular supervised models
...
benchmarkable regulatory-genomics analysis. Future work will complete the full
public biological benchmark, strengthen the supervised and hierarchical
differential-binding models, and integrate attribution-based discovery.
