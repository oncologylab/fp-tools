...
optimistic bias, every reported integrated score is an \emph{out-of-fold}
prediction from five-fold stratified cross-validation: coefficients are learned on
four folds and applied to the held-out fold, so no region contributes to its own
prediction. This is a within-task out-of-fold evaluation restricted to
chromosome~4; it is deliberately distinguished from the stricter
chromosome-held-out and whole-genome cross-validation we identify as the next
validation step. For tasks without BAM-derived cut-site footprints, the integrated
