...
\texttt{--normalization sample-quantile}, each replicate is first mapped to a
shared empirical reference distribution,
\[
\tilde{x}_{s,i}=Q_\ast\{F_s(x_{s,i})\},
\]
where $F_s$ is the empirical CDF of sample $s$ and $Q_\ast$ is the average
reference quantile function across samples. The default
\texttt{condition-quantile} mode preserves the previous behavior by fitting the
quantile transform after condition-level aggregation; \texttt{none} disables this
step.
...
real ATF7-associated sites from the local benchmark fixtures, then created two
deterministic depth-offset replicate profiles per condition to stress-test the
normalization step. In the raw corrected profiles, the controlled baseline
differences are clearly visible; after sample-level quantile normalization, the
baseline offset is reduced while the central cut-site depletion remains visible
...
matrices were aggregated around ATF7-associated sites and converted into two
deterministic depth-offset replicate profiles per condition. Condition means are
shown with light replicate-variation ribbons. The raw panel shows a large
condition-level baseline offset, whereas sample-level quantile normalization
reduces this offset while retaining the central cut-site depletion pattern
expected of a footprint. Generated by
