...
real IRF4- and ETS1-associated sites from the local benchmark fixtures, then created two
deterministic, modestly depth-offset replicate profiles per condition to stress-test the
normalization step without creating unrealistic sample differences. After
sample-level quantile normalization, the corrected cut-site aggregates show the
direction of the normalized footprint
contrast: IRF4 has stronger B-cell center depletion, whereas ETS1 has stronger
...
matrices were aggregated around IRF4-associated sites (a normalized B-cell-deeper example)
and ETS1-associated sites (a normalized T-cell-deeper example), then converted into two
deterministic, modestly depth-offset replicate profiles per condition. Condition means are
shown with light replicate-variation ribbons. The right column uses the same
nonnegative-clamped sample-quantile normalization path used by replicate-aware
\texttt{detect-tf-binding} and \texttt{plot-aggregate}; a lower center relative to
the local flanks indicates stronger TF protection.
Generated by
