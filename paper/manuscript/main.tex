...
aggregated ATACorrect-corrected B-cell and T-cell cut-site tracks around real
ATF7-associated sites from the local benchmark fixtures. In the raw corrected
profiles, condition-level baseline differences remain visible; after
sample-level quantile normalization, the baseline offset is reduced while the
central cut-site depletion remains visible
...
\caption{Effect of sample-level quantile normalization on real corrected cut-site
aggregate profiles. ATACorrect-corrected B-cell and T-cell tracks were
aggregated around ATF7-associated sites. Solid colored lines show the corrected
profiles for each condition. The raw panel shows a condition-level baseline
offset, whereas sample-level quantile normalization reduces this offset while
retaining the central cut-site depletion pattern expected of a footprint.
Generated by
