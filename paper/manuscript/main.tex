...
aggregated the observed B-cell and T-cell replicate footprint tracks around real
TFAP2A-associated sites from the local benchmark fixtures. In the raw profiles,
condition-level baseline differences dominate the comparison; after sample-level
quantile normalization, the baseline offset is reduced while the central
footprint depletion remains visible (Figure~\ref{fig:normalization_effect}).
...
\caption{Effect of sample-level quantile normalization on real replicate footprint profiles.
Observed B-cell and T-cell replicate footprint tracks were aggregated around
TFAP2A-associated sites. Solid colored lines show individual replicates and dotted
lines of the same width show condition means. The raw panel shows a condition-level baseline
offset, whereas sample-level quantile normalization reduces this offset while
retaining the central depletion pattern expected of a footprint. Generated by
