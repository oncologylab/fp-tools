...
used a controlled aggregate-profile simulation with biologically plausible
labels: IRF4 as a B-cell-deeper example and ATF7 as a T-cell-deeper example. The
simulation creates two deterministic, moderately depth-offset replicate profiles
per condition to stress-test the normalization step without creating unrealistic
sample differences. After sample-level quantile normalization, raw baseline
offsets are strongly reduced while the center-versus-flank depletion direction
remains visible (Figure~\ref{fig:normalization_effect}).
...
corrected cut-site aggregate profiles. Controlled B-cell and T-cell aggregate profiles were simulated with IRF4 as a
B-cell-deeper example and ATF7 as a T-cell-deeper example, then converted into two
deterministic, moderately depth-offset replicate profiles per condition. Condition means are
