...
def plot_interactive_bindetect(motifs, comparison, html_out, aggregate_data=None, title='Differential footprint report', report_label=None):
...
function renderColorControls(){colorControls.innerHTML=payload.groups.map(group=>`<label class="color-row"><span>${escText(group)}</span><input type="color" data-color-group="${escText(group)}" value="${payload.colors[group]}"></label>`).join('');colorControls.querySelectorAll('input').forEach(inp=>inp.addEventListener('input',()=>{renderVolcano();renderAggregate(selectedPrefix)}))}function setAggregateLayout(mode){mainRow.classList.toggle('aggregate-wide',mode==='wide');mainRow.classList.toggle('aggregate-full',mode==='full');renderAggregate(selectedPrefix)}function lineDash(type){return type==='dash'?'6 4':(type==='dot'?'1.2 3':'')}function lineWidthValue(input,fallback){const v=Number(input&&input.value);return Number.isFinite(v)&&v>0?Math.min(8,Math.max(.1,v)):fallback}function alphaValue(input,fallback){const v=Number(input&&input.value);return Number.isFinite(v)?Math.min(1,Math.max(0.05,v)):fallback}function dashAttr(type){const dash=lineDash(type);return dash?` stroke-dasharray="${dash}"`:''}const sampleLineStyles={};function sampleStyleKey(name){return String(name||'sample').replace(/[^A-Za-z0-9_.-]+/g,'_')}function sampleLineStyle(name,defaults={}){const key=sampleStyleKey(name),stored=sampleLineStyles[key]||{};return{color:stored.color||defaults.color||'#2563eb',alpha:stored.alpha??0.9,width:stored.width||.7,type:stored.type||'solid'}}function setSampleLineStyle(name,patch){const key=sampleStyleKey(name);sampleLineStyles[key]={...sampleLineStyle(name),...patch}}function renderSampleStyleControls(motif){const rows=[],colors=currentConditionColors();(motif.conditions||[]).forEach((cond,idx)=>{const defaultColor=colors[cond.name]||['#2563eb','#dc2626','#16a34a','#9333ea'][idx%4];rows.push(`<div class="sample-style-group" data-sample-group="${escText(cond.name)}"><div class="sample-style-group-title"><span class="sample-style-dot" style="background:${defaultColor}"></span><span>${escText(cond.name)} samples</span></div><div class="sample-style-row sample-style-head"><span>Sample</span><span>Color</span><span>Alpha</span><span>Width</span><span>Line</span></div>`);(cond.samples||[]).forEach(sample=>{const style=sampleLineStyle(sample.name,{color:defaultColor});rows.push(`<label class="sample-style-row" title="Adjust ${escText(cond.name)} sample ${escText(sample.name)}"><span class="sample-style-name">${escText(sample.name)}</span><input data-sample-color="${escText(sample.name)}" type="color" aria-label="Color for ${escText(cond.name)} sample ${escText(sample.name)}" title="${escText(sample.name)} color" value="${style.color}"><input data-sample-alpha="${escText(sample.name)}" type="number" aria-label="Alpha for ${escText(cond.name)} sample ${escText(sample.name)}" title="${escText(sample.name)} alpha" min="0.05" max="1" step="0.05" value="${style.alpha}"><input data-sample-width="${escText(sample.name)}" type="number" aria-label="Line width for ${escText(cond.name)} sample ${escText(sample.name)}" title="${escText(sample.name)} width" min="0.2" max="5" step="0.1" value="${style.width}"><select data-sample-type="${escText(sample.name)}" aria-label="Line type for ${escText(cond.name)} sample ${escText(sample.name)}" title="${escText(sample.name)} line type"><option value="solid"${style.type==='solid'?' selected':''}>Solid</option><option value="dash"${style.type==='dash'?' selected':''}>Dash</option><option value="dot"${style.type==='dot'?' selected':''}>Dot</option></select></label>`)}) ;rows.push('</div>')});aggregateSampleStyles.innerHTML=rows.join('');aggregateSampleStyles.querySelectorAll('[data-sample-color]').forEach(el=>{el.addEventListener('input',()=>{setSampleLineStyle(el.dataset.sampleColor,{color:el.value});renderAggregate(selectedPrefix,{skipStyleControls:true})})});aggregateSampleStyles.querySelectorAll('[data-sample-alpha]').forEach(el=>{el.addEventListener('input',()=>{setSampleLineStyle(el.dataset.sampleAlpha,{alpha:alphaValue(el,.9)});renderAggregate(selectedPrefix,{skipStyleControls:true})})});aggregateSampleStyles.querySelectorAll('[data-sample-width]').forEach(el=>{el.addEventListener('input',()=>{setSampleLineStyle(el.dataset.sampleWidth,{width:lineWidthValue(el,.7)});renderAggregate(selectedPrefix,{skipStyleControls:true})})});aggregateSampleStyles.querySelectorAll('[data-sample-type]').forEach(el=>{el.addEventListener('change',()=>{setSampleLineStyle(el.dataset.sampleType,{type:el.value});renderAggregate(selectedPrefix,{skipStyleControls:true})})})}aggregateWidth.addEventListener('change',()=>setAggregateLayout(aggregateWidth.value));[aggregateShowMean,aggregateMeanWidth,aggregateMeanType].forEach(el=>el.addEventListener('change',()=>renderAggregate(selectedPrefix)));aggregateMeanWidth.addEventListener('input',()=>renderAggregate(selectedPrefix));
function renderVolcano(){const colors=currentGroupColors(),width=980,height=620,margin={top:58,right:54,bottom:72,left:92},innerW=width-margin.left-margin.right,innerH=390,plotX0=margin.left,plotY0=margin.top,plotX1=plotX0+innerW,plotY1=plotY0+innerH;const xs=payload.points.map(p=>p.change),ys=payload.points.map(p=>p.neglog10p),xabs=Math.max(1,Math.abs(Math.min(...xs,0)),Math.abs(Math.max(...xs,0)))*1.1,ymin=0,ymax=Math.max(1,Math.max(...ys,1)*1.08);const sx=x=>plotX0+((x+xabs)/(2*xabs))*innerW,sy=y=>plotY1-((y-ymin)/(ymax-ymin||1))*innerH,xTicks=niceTicks(-xabs,xabs,7),yTicks=niceTicks(ymin,ymax,7);const parts=[`<rect width="${width}" height="${height}" fill="#ffffff"/>`,`<text x="${(plotX0+plotX1)/2}" y="32" class="plot-title" text-anchor="middle">Differential footprint evidence</text>`,`<rect x="${plotX0}" y="${plotY0}" width="${innerW}" height="${innerH}" fill="#fbfdff" stroke="#d9e2ec"/>`];yTicks.forEach(v=>parts.push(`<line x1="${plotX0}" y1="${sy(v)}" x2="${plotX1}" y2="${sy(v)}" class="grid"/>`,`<text x="${plotX0-12}" y="${sy(v)+4}" class="tick" text-anchor="end">${v.toFixed(1)}</text>`));xTicks.forEach(v=>parts.push(`<line x1="${sx(v)}" y1="${plotY0}" x2="${sx(v)}" y2="${plotY1}" class="grid"/>`,`<text x="${sx(v)}" y="${plotY1+25}" class="tick" text-anchor="middle">${fmtTick(v)}</text>`));parts.push(`<line x1="${sx(0)}" y1="${plotY0}" x2="${sx(0)}" y2="${plotY1}" class="zero"/>`,`<line x1="${plotX0}" y1="${plotY1}" x2="${plotX1}" y2="${plotY1}" class="axis"/>`,`<line x1="${plotX0}" y1="${plotY0}" x2="${plotX0}" y2="${plotY1}" class="axis"/>`,`<text x="${(plotX0+plotX1)/2}" y="${plotY1+62}" class="axis-label" text-anchor="middle">Differential footprint score</text>`,`<text x="28" y="${plotY0+innerH/2}" class="axis-label" text-anchor="middle" transform="rotate(-90 28 ${plotY0+innerH/2})">-log10(p-value)</text>`,`<text x="${plotX0+18}" y="${plotY0+24}" font-family="Arial,Helvetica,sans-serif" font-size="13" font-weight="900" fill="${colors[payload.conditions[1]+'_up']}">${escText(payload.conditions[1]+'_up')}</text>`,`<text x="${plotX1-18}" y="${plotY0+24}" text-anchor="end" font-family="Arial,Helvetica,sans-serif" font-size="13" font-weight="900" fill="${colors[payload.conditions[0]+'_up']}">${escText(payload.conditions[0]+'_up')}</text>`);const volcanoPoints=payload.points.map((p,idx)=>({p,idx,selected:p.prefix===selectedPrefix}));volcanoPoints.sort((a,b)=>Number(a.selected)-Number(b.selected));volcanoPoints.forEach(item=>{const p=item.p,idx=item.idx,selected=item.selected;parts.push(`<circle class="pt${selected?' selected':''}" data-prefix="${escText(p.prefix)}" data-index="${idx}" cx="${sx(p.change).toFixed(2)}" cy="${sy(p.neglog10p).toFixed(2)}" r="${selected?7.0:4.3}" fill="${colors[p.group]||colors['n.s.']}" fill-opacity="${selected?.98:.76}" stroke="${selected?'#111827':'#ffffff'}" stroke-width="${selected?2.8:.9}"/>`)});chart.innerHTML=parts.join('');chart.querySelectorAll('.pt').forEach(el=>el.addEventListener('click',()=>setSelectedMotif(el.dataset.prefix,{from:'volcano'})))}
