...
function targetPlotCount(){return Math.max(1,Math.min(12,Number(plotCountSel.value)||4))}function visiblePanelPrefixes(){return panelPrefixes.slice(0,targetPlotCount())}function gridShape(n){const count=Math.max(1,Math.min(12,Number(n)||4));if(count<=1)return{cols:1,rows:1};if(count<=2)return{cols:2,rows:1};if(count<=4)return{cols:2,rows:2};if(count<=6)return{cols:3,rows:2};if(count<=8)return{cols:4,rows:2};if(count<=9)return{cols:3,rows:3};return{cols:4,rows:3}}function setAggregateGridShape(){const shape=gridShape(visiblePanelPrefixes().length||Number(plotCountSel.value)||4);aggregateGrid.style.setProperty('--aggregate-cols',shape.cols);aggregateGrid.style.setProperty('--aggregate-rows',shape.rows)}
function defaultPanelPrefixes(target){const hasAgg=new Set(allAggregateMotifs().map(m=>m.prefix)),points=(payload.points||[]).filter(p=>p.prefix&&hasAgg.has(p.prefix)),negN=Math.floor(target/2),posN=target-negN,positive=points.filter(p=>p.change>0).sort((a,b)=>b.change-a.change||a.pvalue-b.pvalue).slice(0,posN),negative=points.filter(p=>p.change<0).sort((a,b)=>a.change-b.change||a.pvalue-b.pvalue).slice(0,negN),out=[];((payload.aggregate||{}).default_motifs||[]).forEach(prefix=>{if(out.length<target&&hasAgg.has(prefix)&&!out.includes(prefix))out.push(prefix)});[...positive,...negative].forEach(p=>{if(!out.includes(p.prefix))out.push(p.prefix)});points.sort((a,b)=>Math.abs(b.change)-Math.abs(a.change)||a.pvalue-b.pvalue).forEach(p=>{if(out.length<target&&!out.includes(p.prefix))out.push(p.prefix)});allAggregateMotifs().forEach(m=>{if(out.length<target&&!out.includes(m.prefix))out.push(m.prefix)});return out.slice(0,target)}
...
function lineDash(type){return type==='dash'?'6 4':(type==='dot'?'1.2 3':'')}function dashAttr(type){const dash=lineDash(type);return dash?` stroke-dasharray="${dash}"`:''}function lineWidthValue(input,fallback){const v=Number(input&&input.value!==undefined?input.value:input);return Number.isFinite(v)&&v>0?Math.min(8,Math.max(.1,v)):fallback}function alphaValue(input,fallback){const v=Number(input&&input.value!==undefined?input.value:input);return Number.isFinite(v)?Math.min(1,Math.max(0.05,v)):fallback}function sampleStyleKey(name){return String(name||'sample').replace(/[^A-Za-z0-9_.-]+/g,'_')}function sampleLineStyle(name,defaults={}){const key=sampleStyleKey(name),stored=sampleLineStyles[key]||{};return{color:stored.color||defaults.color||'#2563eb',alpha:stored.alpha??0.9,width:stored.width||2,type:stored.type||'solid'}}function setSampleLineStyle(name,patch){const key=sampleStyleKey(name);sampleLineStyles[key]={...(sampleLineStyles[key]||{}),...patch}}
function renderSampleStyleControls(){const rows=[],colors=currentConditionColors(),by={};allAggregateMotifs().forEach(m=>(m.conditions||[]).forEach(c=>{(by[c.name]||(by[c.name]=new Set()));(c.samples||[]).forEach(s=>by[c.name].add(s.name))}));Object.entries(by).forEach(([cond,names],idx)=>{const defaultColor=colors[cond]||['#2563eb','#dc2626','#16a34a','#9333ea'][idx%4];rows.push(`<div class="sample-style-group" data-sample-group="${escText(cond)}"><div class="sample-style-group-title"><span class="sample-style-dot" style="background:${defaultColor}"></span><span>${escText(cond)} samples</span></div><div class="sample-style-row sample-style-head"><span>Show</span><span>Sample</span><span>Color</span><span>Alpha</span><span>Width</span><span>Line</span></div>`);[...names].forEach(name=>{const style=sampleLineStyle(name,{color:defaultColor}),checked=visibleSamples.has(name)?' checked':'',label=sampleDisplayName(name,cond);rows.push(`<label class="sample-style-row" title="Adjust ${escText(label)}"><input class="sample-visible" data-sample-visible="${escText(name)}" type="checkbox" aria-label="Show ${escText(label)}"${checked}><span class="sample-style-name" title="${escText(label)}">${escText(label)}</span><input data-sample-color="${escText(name)}" type="color" aria-label="Color for ${escText(label)}" value="${style.color}"><input data-sample-alpha="${escText(name)}" type="number" aria-label="Alpha for ${escText(label)}" min="0.05" max="1" step="0.05" value="${style.alpha}"><input data-sample-width="${escText(name)}" type="number" aria-label="Line width for ${escText(label)}" min="0.2" max="5" step="0.1" value="${style.width}"><select data-sample-type="${escText(name)}"><option value="solid"${style.type==='solid'?' selected':''}>Solid</option><option value="dash"${style.type==='dash'?' selected':''}>Dash</option><option value="dot"${style.type==='dot'?' selected':''}>Dot</option></select></label>`)});rows.push('</div>')});aggregateSampleStyles.innerHTML=rows.join('');aggregateSampleStyles.querySelectorAll('[data-sample-visible]').forEach(el=>el.addEventListener('change',()=>{if(el.checked)visibleSamples.add(el.dataset.sampleVisible);else visibleSamples.delete(el.dataset.sampleVisible);if(!visibleSamples.size)allSampleLabels().forEach(label=>visibleSamples.add(label));renderAll()}));aggregateSampleStyles.querySelectorAll('[data-sample-color]').forEach(el=>el.addEventListener('input',()=>{setSampleLineStyle(el.dataset.sampleColor,{color:el.value});renderAll(false)}));aggregateSampleStyles.querySelectorAll('[data-sample-alpha]').forEach(el=>el.addEventListener('input',()=>{setSampleLineStyle(el.dataset.sampleAlpha,{alpha:alphaValue(el,.9)});renderAll(false)}));aggregateSampleStyles.querySelectorAll('[data-sample-width]').forEach(el=>el.addEventListener('input',()=>{setSampleLineStyle(el.dataset.sampleWidth,{width:lineWidthValue(el,2)});renderAll(false)}));aggregateSampleStyles.querySelectorAll('[data-sample-type]').forEach(el=>el.addEventListener('change',()=>{setSampleLineStyle(el.dataset.sampleType,{type:el.value});renderAll(false)}))}
function niceStep(raw){if(!Number.isFinite(raw)||raw<=0)return 1;const pow=Math.pow(10,Math.floor(Math.log10(raw))),f=raw/pow;return (f<=1?1:f<=1.5?1.5:f<=2.5?2.5:f<=5?5:10)*pow}function niceLimit(value){const step=niceStep(Math.abs(value)/5);return Math.max(step,Math.ceil(Math.abs(value)/step)*step)}function niceTicks(min,max,n){const step=niceStep((max-min)/Math.max(1,n-1)),start=Math.ceil(min/step)*step,end=Math.floor(max/step)*step,out=[];for(let v=start;v<=end+step/2;v+=step)out.push(Number(v.toPrecision(12)));return out.length?out:[0]}function fmtTick(value){const a=Math.abs(value);if(!Number.isFinite(value))return'';if(a===0)return'0';if(a>=1)return value.toFixed(1).replace('-0.0','0.0');if(a>=.01)return value.toFixed(2).replace('-0.00','0.00');if(a>=.001)return value.toFixed(3).replace('-0.000','0.000');return value.toExponential(1).replace('-0.0e+0','0')}function fmtShort(v){return Math.abs(v)>=1000?`${Math.round(v/100)/10}k`:String(Math.round(v*100)/100)}
...
function drawAggregatePanel(motif,idx){const rawX=(payload.aggregate||{}).x||[],keep=rawX.map((v,i)=>({v,i})).filter(p=>p.v>=-aggregateDisplayBp&&p.v<=aggregateDisplayBp),x=keep.length?keep.map(p=>p.v):rawX,samples=samplesForPanel(motif).map(s=>({...s,profile:keep.length?keep.map(p=>s.profile[p.i]):s.profile})),siteCount=visibleSiteCount(motif,samples),width=300,height=300,margin={top:30,right:8,bottom:34,left:36},innerW=width-margin.left-margin.right,innerH=height-margin.top-margin.bottom,colors=currentConditionColors(),allY=samples.flatMap(s=>s.profile||[]).filter(Number.isFinite);let rawMin=Math.min(...allY,0),rawMax=Math.max(...allY,1e-9),pad=Math.max((rawMax-rawMin||1)*.06,1e-6),step=niceStep((rawMax-rawMin+2*pad)/4),ymin=Math.floor((rawMin-pad)/step)*step,ymax=Math.ceil((rawMax+pad)/step)*step;const sx=v=>margin.left+((v-x[0])/(x[x.length-1]-x[0]||1))*innerW,sy=v=>margin.top+innerH-((v-ymin)/(ymax-ymin||1))*innerH,yTicks=niceTicks(ymin,ymax,4),xTicks=[-aggregateDisplayBp,0,aggregateDisplayBp],sampleSeries=samples.slice().sort((a,b)=>(Number(b.fp_score||0)-Number(a.fp_score||0)));let parts=[`<svg class="aggregate-panel" data-panel="${idx}" viewBox="0 0 ${width} ${height}"><style>${plotSvgStyle}</style><rect width="${width}" height="${height}" fill="#fff"/><text x="${width/2}" y="18" class="plot-title" text-anchor="middle">${escText(motifLabel(motif))}</text>`];yTicks.forEach(v=>parts.push(`<text x="${margin.left-6}" y="${sy(v)+3}" class="tick" text-anchor="end">${fmtTick(v)}</text>`));xTicks.forEach(v=>parts.push(`<line x1="${sx(v)}" y1="${margin.top}" x2="${sx(v)}" y2="${margin.top+innerH}" class="grid"/><text x="${sx(v)}" y="${margin.top+innerH+17}" class="tick" text-anchor="middle">${v}</text>`));parts.push(`<line x1="${sx(0)}" y1="${margin.top}" x2="${sx(0)}" y2="${margin.top+innerH}" class="zero"/><line x1="${margin.left}" y1="${margin.top+innerH}" x2="${margin.left+innerW}" y2="${margin.top+innerH}" class="axis"/><line x1="${margin.left}" y1="${margin.top}" x2="${margin.left}" y2="${margin.top+innerH}" class="axis"/><text x="${margin.left+7}" y="${margin.top+innerH-8}" class="tick" fill="#94a3b8">${siteCount}</text>`);sampleSeries.forEach((s,i)=>{const style=sampleLineStyle(s.name,{color:colors[s.condition]||'#64748b'}),dash=dashAttr(style.type),label=sampleDisplayName(s,s.condition);parts.push(`<path d="${pathD(s.profile,x,sx,sy)}" fill="none" stroke="${style.color}" stroke-width="${lineWidthValue(style.width,2)}"${dash} stroke-opacity="${alphaValue(style.alpha,.9)}"><title>${escText(label)}</title></path>`)});parts.push(`<text x="${margin.left+innerW/2}" y="${height-6}" class="axis-label" text-anchor="middle">${escText((payload.aggregate||{}).x_label||'Distance from motif center (bp)')}</text><text x="10" y="${margin.top+innerH/2}" class="axis-label" text-anchor="middle" transform="rotate(-90 10 ${margin.top+innerH/2})">${escText((payload.aggregate||{}).y_label||'Corrected cut-site signal')}</text></svg>`);return parts.join('')}
