...
def read_diff_footprint_motif_table(results_path: Path, diff_dir: Path, max_motifs: int | None) -> pd.DataFrame:
    ...
    if missing:
        raise SystemExit(f"Differential-footprint results table is missing required columns: {', '.join(sorted(missing))}")
    rows = []
...
def plot_score_grid(
    ...
    fig.suptitle("Per-cell marker footprint signatures", y=1.01)
    fig.tight_layout()
...
def plot_knn_with_reference(
    ...
    fig.suptitle("Per-cell KNN footprint signature scores", y=1.02, fontsize=10.8, fontweight="bold")
...
def plot_per_cell_signature_heatmap(
    ...
    fig.suptitle("Per-cell KNN footprint signature heatmap", y=1.08, fontsize=11.0, fontweight="bold")
    save_illustrator_svg(fig, output_prefix)
    plt.close(fig)
...
def plot_all_motif_per_cell_heatmap(
    ...
    output_prefix: Path,
    *,
    title: str = "All-motif per-cell KNN footprint signatures",
...
def signature_scores_from_matrix(
    ...
    for tf in marker_tfs:
        ...
        if hits.empty:
            raise SystemExit(f"Could not find selected marker TF {tf} in the all-motif heatmap rows.")
        row = hits.iloc[0]
...
def write_top_motif_heatmap(
    ...
    plot_all_motif_per_cell_heatmap(
        ...
        ordered,
        output_prefix,
        title="Top cell-type-specific per-cell KNN footprint signatures",
    ...
    for cell_type in CELL_TYPES:
        ...
        with PdfPages(output_pdf) as pdf:
            for page_start in range(0, subset_meta.shape[0], panels_per_page):
                ...
                fig.suptitle(
                    f"{cell_type} dominant footprint signatures ({page_number}/{total_pages})",
                    fontsize=11.0,
                    fontweight="bold",
                    y=0.98,
                )
...
def score_all_motif_per_cell_heatmap(
    ...
    *,
    ...
    marker_scores: pd.DataFrame | None,
    summary_output_prefix: Path | None,
    summary_markers: list[str],
    top_output_prefix: Path,
    ...
    if summary_output_prefix is not None:
        plot_fig4_single_cell_footprinting(
            annotations,
            signature_scores_from_matrix(top_matrix, top_metadata, ordered, summary_markers),
            top_matrix,
            top_metadata,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--flank-outer", type=int, default=100, help="Outer flank distance from motif center in bp (default: 100).")
    parser.add_argument("--bin-size", type=int, default=500, help="Bin size for the companion chromVAR-like motif activity score (default: 500).")
    parser.add_argument(
        "--marker-groups",
    ...
    parser.add_argument("--summary-output-prefix", dest="summary_output_prefix", help="Output prefix for the combined heatmap and UMAP summary SVG when all-motif heatmap data are available.")
    parser.add_argument("--fig4-output-prefix", dest="summary_output_prefix", help=argparse.SUPPRESS)
    parser.add_argument("--all-tf-review-prefix", help="Output prefix for three multi-page all-TF signature review PDFs grouped by dominant broad cell type.")
    ...
    summary_output_prefix = args.summary_output_prefix or ("Fig4" if args.legacy_pbmc5k_names else "single_cell_footprinting")
    all_tf_review_prefix = args.all_tf_review_prefix or (
    ...
    plot_knn_with_reference(annotations, knn_scores, markers, outdir / f"{legacy_prefix}knn_footprint_signature_umap")
    ...
    plot_single_cell_footprinting_summary(
        ...
        outdir / f"{legacy_prefix}single_cell_footprinting_summary",
    )
    plot_per_cell_signature_heatmap(
        annotations,
        knn_scores,
        summary_markers,
        outdir / f"{legacy_prefix}per_cell_footprint_signature_heatmap",
    )
    plot_score_grid(
        annotations,
        [("ChromVAR-like", chromvar_scores, "chromvar_like_activity_z", "activity z")],
        markers,
        outdir / f"{legacy_prefix}chromvar_like_motif_activity_umap",
    )
    plot_score_grid(
        annotations,
        [
        ...
        markers,
        outdir / f"{legacy_prefix}per_cell_signature_review",
    )
    if args.all_motif_score_table:
        ...
        plot_all_motif_per_cell_heatmap(
            ...
            metadata,
            ordered,
            outdir / f"{legacy_prefix}all_motif_per_cell_footprint_signature_heatmap",
        )
        top_matrix, top_metadata = write_top_motif_heatmap(
            matrix,
            metadata,
            ordered,
            outdir / f"{legacy_prefix}top_motif_per_cell_footprint_signature_heatmap",
    ...
    if args.all_motif_diff_dir or args.all_motif_results:
        ...
        score_all_motif_per_cell_heatmap(
            ...
            marker_scores=marker_scores_for_figures,
            summary_output_prefix=outdir / summary_output_prefix,
            summary_markers=markers,
            top_output_prefix=outdir / f"{legacy_prefix}top_motif_per_cell_footprint_signature_heatmap",
            all_tf_review_prefix=None if args.skip_all_tf_review_pdfs else outdir / all_tf_review_prefix,
            all_tf_review_panels_per_page=args.all_tf_review_panels_per_page,
        )
    print(f"Wrote per-cell footprint-signature plots and score tables to {outdir}")
    return 0
