...
def _has_aggregate_profiles(review_payload: dict) -> bool:
    return any(
        ((item.get("payload") or {}).get("aggregate") or {}).get("motifs")
        for item in review_payload.get("comparisons") or []
    )
...
def write_review_html(
    ...
    has_aggregate_profiles = _has_aggregate_profiles(review_payload)
