...
def build_payload(rows: list[dict[str, str]], flank: int, top_n: int, normalization: str = "none") -> dict:
    ...
    return {"schema": "fp-tools.aggregate.batch.v2", "x": x, "motifs": motifs, "conditions": conditions, "colors": colors, "normalization": normalization, "site_set": _summarize_site_sets([str(m.get("site_set") or "") for m in motifs]), "x_label": "Distance from motif center (bp)", "y_label": "Corrected cut-site signal (a.u.)" if normalization == "none" else "Normalized corrected cut-site signal (a.u.)"}
...
def _series_from_diff_payload(payload: dict, source_label: str) -> dict:
    ...
    for motif in aggregate.get("motifs") or []:
        ...
        motifs.append({"prefix": str(motif.get("prefix") or motif.get("name") or "motif"), "name": str(motif.get("name") or motif.get("prefix") or "motif"), "motif_id": str(motif.get("motif_id") or ""), "score": abs(float(motif.get("change") or 0.0)), "sites": int(motif.get("n_sites") or motif.get("sites") or 0), "site_set": bed_source, "series": series})
