...
function sampleStyle(label,defaults={{}}){{const stored=sampleLineStyles[label]||{{}};return{{color:stored.color||defaults.color||'#2563eb',alpha:stored.alpha??0.9,width:stored.width||0.7,type:stored.type||'solid'}}}}
function renderSampleStyles(){{const colors=currentColors();sampleStyleControls.innerHTML=allSamples().map(s=>{{const style=sampleStyle(s.label,{{color:colors[s.condition]}});return `<label class="sample-style-row" title="Adjust ${{escText(s.label)}}"><span class="sample-style-name">${{escText(s.label)}}</span><input data-sample-color="${{escText(s.label)}}" type="color" aria-label="Color for ${{escText(s.label)}}" value="${{style.color}}"><input data-sample-alpha="${{escText(s.label)}}" type="number" min="0.05" max="1" step="0.05" value="${{style.alpha}}"><input data-sample-width="${{escText(s.label)}}" type="number" min="0.2" max="5" step="0.1" value="${{style.width}}"><select data-sample-type="${{escText(s.label)}}"><option value="solid"${{style.type==='solid'?' selected':''}}>Solid</option><option value="dash"${{style.type==='dash'?' selected':''}}>Dash</option><option value="dot"${{style.type==='dot'?' selected':''}}>Dot</option></select></label>`}}).join('');sampleStyleControls.querySelectorAll('[data-sample-color]').forEach(el=>el.addEventListener('input',()=>{{sampleLineStyles[el.dataset.sampleColor]={{...sampleStyle(el.dataset.sampleColor),color:el.value}};renderAll(false)}}));sampleStyleControls.querySelectorAll('[data-sample-alpha]').forEach(el=>el.addEventListener('input',()=>{{sampleLineStyles[el.dataset.sampleAlpha]={{...sampleStyle(el.dataset.sampleAlpha),alpha:alphaValue(el.value,.9)}};renderAll(false)}}));sampleStyleControls.querySelectorAll('[data-sample-width]').forEach(el=>el.addEventListener('input',()=>{{sampleLineStyles[el.dataset.sampleWidth]={{...sampleStyle(el.dataset.sampleWidth),width:lineWidthValue(el.value,.7)}};renderAll(false)}}));sampleStyleControls.querySelectorAll('[data-sample-type]').forEach(el=>el.addEventListener('change',()=>{{sampleLineStyles[el.dataset.sampleType]={{...sampleStyle(el.dataset.sampleType),type:el.value}};renderAll(false)}}))}}
...
function fmt(v){{return Math.abs(v)>=1?v.toFixed(1).replace('-0.0','0.0'):v.toFixed(2).replace('-0.00','0.00')}}
...
function lineWidthValue(v,fallback){{const n=Number(v);return Number.isFinite(n)&&n>0?Math.min(8,Math.max(.1,n)):fallback}}
function alphaValue(v,fallback){{const n=Number(v);return Number.isFinite(n)?Math.min(1,Math.max(.05,n)):fallback}}
function sampleStyle(label,defaults={{}}){{const stored=sampleLineStyles[label]||{{}};return{{visible:stored.visible!==false,color:stored.color||defaults.color||'#2563eb',alpha:stored.alpha??0.9,width:stored.width||0.7,type:stored.type||'solid'}}}}
...
function bindColumnEvents(){{motifColumns.querySelectorAll('[data-col]').forEach(col=>col.addEventListener('click',ev=>{{if(ev.target.closest('select,input,button'))return;activeSlot=Number(col.dataset.col);renderAll(false)}}));motifColumns.querySelectorAll('[data-motif]').forEach(sel=>sel.addEventListener('change',()=>{{slotPrefixes[Number(sel.dataset.motif)]=sel.value;activeSlot=Number(sel.dataset.motif);groupAutoscaleDomain=null;renderAll(false)}}));motifColumns.querySelectorAll('[data-group]').forEach(inp=>inp.addEventListener('change',()=>{{const idx=Number(inp.dataset.group);if(inp.checked)groupAutoscalePanels.add(idx);else groupAutoscalePanels.delete(idx);groupAutoscaleDomain=null;renderAll(false)}}));motifColumns.querySelectorAll('[data-visible]').forEach(inp=>inp.addEventListener('change',()=>{{const [idx,label]=inp.dataset.visible.split(':');slotSampleStyles[Number(idx)][label]={{...sampleStyle(Number(idx),label),visible:inp.checked}};renderAll(false)}}));motifColumns.querySelectorAll('[data-color]').forEach(inp=>inp.addEventListener('input',()=>{{const [idx,label]=inp.dataset.color.split(':');slotSampleStyles[Number(idx)][label]={{...sampleStyle(Number(idx),label),color:inp.value}};renderAll(false)}}));motifColumns.querySelectorAll('[data-alpha]').forEach(inp=>inp.addEventListener('input',()=>{{const [idx,label]=inp.dataset.alpha.split(':');slotSampleStyles[Number(idx)][label]={{...sampleStyle(Number(idx),label),alpha:alphaValue(inp.value,.9)}};renderAll(false)}}));motifColumns.querySelectorAll('[data-width]').forEach(inp=>inp.addEventListener('input',()=>{{const [idx,label]=inp.dataset.width.split(':');slotSampleStyles[Number(idx)][label]={{...sampleStyle(Number(idx),label),width:lineWidthValue(inp.value,.7)}};renderAll(false)}}));motifColumns.querySelectorAll('[data-type]').forEach(sel=>sel.addEventListener('change',()=>{{const [idx,label]=sel.dataset.type.split(':');slotSampleStyles[Number(idx)][label]={{...sampleStyle(Number(idx),label),type:sel.value}};renderAll(false)}}));motifColumns.querySelectorAll('[data-download-panel]').forEach(btn=>btn.addEventListener('click',ev=>{{ev.stopPropagation();const svg=document.querySelector(`.aggregate-panel[data-panel="${{btn.dataset.downloadPanel}}"]`);if(svg)downloadBlob(svgBlob(svg),`plot_aggregate_panel_${{Number(btn.dataset.downloadPanel)+1}}.svg`)}}))}}
