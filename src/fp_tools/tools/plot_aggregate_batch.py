...
from fp_tools.utils.report_logos import MOTIF_LOGO_JAVASCRIPT
...
def _selected_motif_matrices(payload: dict, prefixes: set[str]) -> dict:
    """Retain usable A/C/G/T count matrices without invalidating profile data."""
    matrices = payload.get("motif_matrices")
    if not isinstance(matrices, dict):
        return {}
    selected = {}
    for prefix in prefixes:
        try:
            matrix = np.asarray(matrices.get(prefix), dtype=float)
        except (TypeError, ValueError, OverflowError):
            continue
        if (matrix.ndim == 2 and matrix.shape[0] == 4 and matrix.shape[1] > 0
                and np.isfinite(matrix).all() and (matrix >= 0).all()):
            selected[prefix] = matrix.tolist()
    return selected
...
def merge_payloads(payloads: list[dict]) -> dict:
    ...
    for payload in normalized:
        ...
        merged["motif_matrices"].update(
            _selected_motif_matrices(payload, {str(m["prefix"]) for m in payload["motifs"]})
        )
...
{MOTIF_LOGO_JAVASCRIPT}
function logoHtml(prefix){{return motifLogoHtml(prefix)}}
...
function downloadLogoPanel(){{const w=230,h=132,gap=8,totalW=slotPrefixes.length*w+(slotPrefixes.length-1)*gap;let parts=[`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${{totalW}} ${{h}}" font-family="Arial,Helvetica,sans-serif"><rect width="100%" height="100%" fill="#fff"/>`];slotPrefixes.forEach((prefix,i)=>{{const motif=motifByPrefix(prefix),x=i*(w+gap),logoUri=logoDataUri(prefix);parts.push(`<g transform="translate(${{x}},0)"><rect width="${{w}}" height="${{h}}" rx="7" fill="#fff" stroke="#d9e2ec"/><text x="8" y="18" font-size="12" font-weight="900" fill="#152133">${{escText(motifLabel(motif)).slice(0,30)}}</text>`);if(logoUri)parts.push(`<image x="12" y="28" width="${{w-24}}" height="90" preserveAspectRatio="xMidYMid meet" href="${{logoUri}}"/>`);else parts.push(`<text x="${{w/2}}" y="76" text-anchor="middle" font-size="11" font-weight="800" fill="#94a3b8">Motif logo unavailable</text>`);parts.push('</g>')}});parts.push('</svg>');downloadBlob(new Blob(parts,{{type:'image/svg+xml;charset=utf-8'}}),'plot_aggregate_motif_logo_panel.svg')}}
function updateGroupAutoscale(){{const panels=[...groupAutoscalePanels];if(!panels.length){{groupAutoscaleDomain=null;renderAll(false);return}}let vals=[];panels.forEach(idx=>{{const motif=motifByPrefix(slotPrefixes[idx]);samplesForMotif(motif,idx).forEach(s=>vals=vals.concat(s.profile.filter(Number.isFinite)))}});let rawMin=Math.min(...vals,0),rawMax=Math.max(...vals,1e-9),pad=Math.max((rawMax-rawMin||1)*.06,1e-6),step=niceStep((rawMax-rawMin+2*pad)/4),ymin=Math.floor((rawMin-pad)/step)*step,ymax=Math.ceil((rawMax+pad)/step)*step;groupAutoscaleDomain=[ymin,ymax];renderAll(false)}}
