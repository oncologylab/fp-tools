...
from fp_tools.utils.motif_databases import DEFAULT_MOTIF_DB, motif_db_table
from fp_tools.utils.references import is_managed_assembly, resolve_analysis_reference
...
def _run(command: list[str], stdout_path: Path, stderr_path: Path) -> int:
    ...
    result = run_logged(resolved, stdout_log=stdout_path, stderr_log=stderr_path, env=env, label=command[0])
...
def build_commands(args: argparse.Namespace) -> list[tuple[str, list[str]]]:
    ...
    args.cores = resolve_cores(args.cores, warn=lambda message: print(message, file=sys.stderr, flush=True))
    ...
    if not motif_args:
        motif_args = ["--motif-db", DEFAULT_MOTIF_DB]
    atac = [
        "atac-correct",
        *shared,
        "--genome",
...
def run_bulk_footprinting(args: argparse.Namespace) -> int:
    ...
    resolved_args.cores = resolve_cores(args.cores, warn=lambda message: print(message, file=sys.stderr, flush=True))
    resolved_args.genome = str(reference.fasta)
    resolved_args.blacklist = (
        str(reference.blacklist) if reference.blacklist is not None else None
    )
    project = Path(args.outdir).expanduser().resolve()
    review_format = _resolve_review_format(resolved_args)
    commands = build_commands(resolved_args)
    print(f"[resources] Using {resolved_args.cores} worker cores", flush=True)
    print(f"[reference] {reference.assembly or 'custom'}: {reference.fasta}", flush=True)
    print(f"[blacklist] {reference.blacklist or 'disabled'}", flush=True)
    if args.dry_run:
        for command_label, command in commands:
            print(f"[{command_label}] {_quote(command)}")
        return 0
    ...
    for label, command in commands:
        if args.resume and not args.force and _stage_complete(
            ...
            print(f"[resume] {label}: complete", flush=True)
            continue
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--genome", required=True, help="Managed assembly (hg38 or mm10) or a reference FASTA matching the BAM and peak coordinates.")
    parser.add_argument(
        "--reference-dir",
        help="Managed reference cache root (default: ~/.cache/fp-tools/references).",
    )
    blacklist_group = parser.add_mutually_exclusive_group()
    blacklist_group.add_argument("--blacklist", help="Blacklist BED overriding the managed assembly blacklist.")
    blacklist_group.add_argument(
        "--no-blacklist",
        action="store_true",
        help="Disable the automatic hg38/mm10 blacklist.",
    )
    parser.add_argument("--motifs", nargs="*", help="Optional motif files.")
    parser.add_argument("--motif-db", help=f"Built-in motif database to use alone or combine with --motifs (default when neither is provided: {DEFAULT_MOTIF_DB}).")
    parser.add_argument(
        "--list-motif-dbs",
        action="store_true",
        help="List available built-in motif databases and exit.",
    )
    ...
    parser.add_argument("--cores", type=int, default=None, help="Optional total worker core limit (default: all available cores).")
...
def _preflight_input_paths(args: argparse.Namespace) -> None:
    ...
    if not is_managed_assembly(args.genome):
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (ValueError, OSError, RuntimeProvisionError) as exc:
