...
"""fp-tools command-line argument builders."""
...
def add_atacorrect_arguments(parser):
	...
	optargs.add_argument('--write-tracks', metavar="<track>", nargs="*", choices=["corrected", "uncorrected", "bias", "expected", "all"], default=["corrected"], help="Cut-site signal bigWigs to write (default: corrected; use all for corrected, observed/uncorrected, sequence-bias, and expected signals)")
	...
	runargs.add_argument('--sample-table', metavar="<tsv>", help="Project sample table with sample, condition, bam, and peaks columns")
	runargs.add_argument('--layout', choices=["custom", "project"], default="project", help="Use fp-tools standard project output layout under --outdir (default: project when --sample-table is provided)")
	runargs.add_argument('--sample-output-root', metavar="<directory>", help="Sample output root; writes each sample under <root>/<sample>/atac_correct, typically <project>/samples")
...
def add_scorebigwig_arguments(parser):
	...
	multiscaleargs = parser.add_argument_group('Parameters for score == multiscale')
	...
	runargs = parser.add_argument_group('Run arguments')
	runargs.add_argument('--sample-names', metavar="<name>", nargs="*", help="Sample labels for --signals when using project layout")
	runargs.add_argument('--sample-table', metavar="<tsv>", help="Project sample table with sample, condition, bam, and peaks columns")
	runargs.add_argument('--layout', choices=["custom", "project"], default="project", help="Use fp-tools standard project output layout under --outdir (default: project when --sample-table is provided)")
	runargs.add_argument('--sample-output-root', metavar="<directory>", help="Sample output root; writes each sample under <root>/<sample>/footprints, typically <project>/samples")
...
def add_diff_footprints_arguments(parser, command_name="diff-footprints"):
	...
	optargs.add_argument('--plot-aggregate-motifs', metavar="<motif>", nargs="+", help="Ordered motif IDs, names, or output prefixes to embed as aggregate profiles; overrides automatic aggregate-motif selection")
	optargs.add_argument('--default-aggregate-plots', metavar="<int>", type=int, default=4, help="Number of aggregate profiles initially displayed in interactive reports (default: 4; maximum: 12)")
...
def add_tfbscan_arguments(parser):
	...
	description += "Usage:\nfp-tools TFBScan --motifs <motifs.txt> --fasta <genome.fa> \n\n"
...
def add_aggregate_arguments(parser):
	...
	parser.formatter_class = lambda prog: argparse.RawDescriptionHelpFormatter(prog, max_help_position=40, width=90)
	description = ""
	parser.description = format_help_description("plot-aggregate", description)
	...
	IO.add_argument('--sample-table', metavar="<tsv>", help="Project sample table with sample and condition columns")
	IO.add_argument('--layout', choices=["custom", "project"], default="project", help="Use fp-tools standard project output layout under --outdir (default: project when --sample-table is provided)")
...
def add_heatmap_arguments(parser):
	...
	IO.add_argument('--output',  metavar="", help="Output filename (default: fp-tools_heatmap.pdf)", default="fp-tools_heatmap.pdf")
	...
	PLOT.add_argument('--title', metavar="", default="fp-tools heatmap")
...
def add_maxpos_arguments(parser):
	...
	description += "Usage:\nfp-tools MaxPos --bed <regions.bed> --bigwig <signal.bw>\n\n"
...
def add_motifclust_arguments(parser):
	...
	description += "Usage:\nfp-tools ClusterMotifs --motifs <motifs.jaspar>\n\n"
...
def add_downloaddata_arguments(parser):
	...
	parser.formatter_class = lambda prog: argparse.RawDescriptionHelpFormatter(prog, max_help_position=40, width=90)
	description = "Download test data for the fp-tools command-line examples"
	...
	arguments.add_argument('--bucket', metavar="", help="Name of bucket to download (default: data-fptools-2020)", default="data-fptools-2020")
